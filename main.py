...
class Transcriber:
    ...
    def split_into_smaller_chunks(self,tempdir, audio_file):
        "Split up audio using ffmpeg and save to temp directory"
        output_file = os.path.join(tempdir,"audio")
        process = sp.Popen(
                [
                "ffmpeg","-i",
                audio_file,
    ...
    def transcribe_temp_audio_file(self, file,lang):
    ...
    def print_files_found(self, audio_files):
    ...
    def simple_cli(self, lang):
        "main function that calls other functions"
        start_time = time.perf_counter()
        audio_files = self.get_audio_files()
        self.print_files_found(audio_files)

        for audio_file in audio_files:
            start_time_for_file =time.perf_counter()
            with tempfile.TemporaryDirectory() as tempdir:
                ori_length = self.split_into_smaller_chunks(tempdir,audio_file)
                if ori_length != None:
                    ...
                    self.printProgressBar(0,len(full_paths),prefix="TRANSCRIBING",suffix=audio_file,length=50,printEnd="\r")
                    for i, file in enumerate(full_paths):
                        trans_results = self.transcribe_temp_audio_file(file, lang)
                        while trans_results == None:
                            ...
                            if answer.lower().strip() == 'y' or answer.lower().strip() == 'yes':
                                ...
                                break
                        self.printProgressBar(i+1,len(full_paths),prefix="TRANSCRIBING",suffix=audio_file,length=50,printEnd = "\r")
                        self.write_to_txt(txt_file_name,trans_results)
...
if __name__ == "__main__":
    ...
    transcriber.simple_cli(lang="id-ID")
